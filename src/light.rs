    ...
    pub fn rec(&self, rec: &mut rerun::RecordingStream) {
        match self {
            Self::Area {
                ...
                let q = transform.transform_point(&Point3::new(-(size / 2.0), 0.0, -(size / 2.0)));
                let u =
                    transform.transform_point(&(Point3::new(size / 2.0, 0.0, -(size / 2.0)))) - q;
                let v =
                    transform.transform_point(&(Point3::new(-(size / 2.0), 0.0, size / 2.0))) - q;
        ...
        match self {
            Self::Area {
                ...
                quad_ray_intersection(q, u, v, light_ray, interval).map(|light_t| {
                    ...
                    let world_p = self.transform().transform_point(&light_p);
